...
const panels = [
  ...
  { id: 'grievances', label: 'All Grievances', icon: '📋', section: null },
...
export default function AdminDashboard() {
  ...
  const titles = { home: 'Admin Dashboard', grievances: 'All Grievances', citizens: 'Citizens', reports: 'Reports & Analytics', settings: 'Settings' };

  const renderPanel = () => {
    switch (activePanel) {
      case 'home': return <ADHome />;
      case 'grievances': return <ADGrievances />;
      case 'citizens': return <ADCitizens />;
      case 'reports': return <ADReports />;
      case 'settings': return <ADSettings />;
      default: return null;
    }
  };
    ...
    <div className="dashboard">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          {panels.map(p => (
            <div key={p.id}>
              {p.section && <div className="sidebar-section">{p.section}</div>}
              <button className={`sidebar-item ${activePanel === p.id ? 'active' : ''}`} onClick={() => setActivePanel(p.id)}>
        ...
        <div className="sidebar-footer">
          <div className="user-info">
            <div className="user-avatar" style={{ background: 'linear-gradient(135deg,var(--gold),var(--crimson))' }}>A</div>
            <div>
      ...
      <main className="dash-main">
        <div className="dash-topbar">
          <div>
            ...
            <div className="dash-page-sub">System overview and key metrics</div>
          </div>
          <div className="dash-topbar-actions">
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, color: 'var(--text-mid)' }}>
